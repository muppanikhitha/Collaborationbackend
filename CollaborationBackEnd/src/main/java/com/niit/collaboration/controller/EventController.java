...
public class EventController {
	...
	/**
	 * 	http://localhost:8020/CollaborationFrontEnd/events									[working]
	 * @return
	 */
	...
	/**
	 * 	http://localhost:8020/CollaborationFrontEnd/event/									[working]
	 * @param event
	 * @return
	 */
	...
	public ResponseEntity<Event> createEvent(@RequestBody Event event) {
		...
		event.setErrMessage("Event already exist with id : " +event.getId());
		log.error("Event already exist with id : " +event.getId());
		return new ResponseEntity<Event>(HttpStatus.OK);
	}
	
	/**
	 * 	http://localhost:8020/CollaborationFrontEnd/event/{id}								[working]
	 * @param id
	 * @param event
	 * @return
	 */
	...
	public ResponseEntity<Event> updateEvent(@PathVariable("id") int id, @RequestBody Event event) {
		...
		if(eventDAO.get(id) == null) {
			event = new Event();
			event.setErrMessage("No event exist with id : " +event.getId());
	...
	public ResponseEntity<Event> deleteEvent(@PathVariable("id") int id) {
		...
		if(event == null) {
			event = new Event();
			event.setErrMessage("No event exist with id : " + id);
